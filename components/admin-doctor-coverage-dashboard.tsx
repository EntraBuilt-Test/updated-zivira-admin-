...
import type { ZiviraTreeNode, DoctorCoverageRow } from "@zivira/types";
import { apiClient } from "@/lib/api-client";
import { downloadCsv } from "@/lib/download-csv";

// Fix — this page used to be a fully static server component: none of its
// buttons/selects/inputs (Export, Add HCP, search, the dropdown filters,
...
// Managers, and pagination) did anything when clicked. A later pass wired
// all of that up against a local mock doctor array. This pass replaces
// that mock array with a real fetch from GET /company/doctor-coverage
// (apiClient.doctorCoverage), keeping every filter/search/pagination/
// export/detail-popup working against the real DoctorCoverageRow data.
//
// DoctorCoverageRow has no "hospital", "tier/call-frequency target",
// "planned visits", or "products given" concept — those were fabricated
// mock fields. They are dropped/relabeled below: the Tier filter and the
// Planned-vs-Actual visits column are replaced with real fields the
// backend does return — totalVisits, totalSamples, totalGifts,
// totalGiftValueRs and overGiftThreshold (the real MCI gift-value alert).
// The 4 demo KPI numbers at the top and the static specialty/territory
// analytics panels at the bottom are left as-is (no backend collection
// exists yet for those pan-India rollups). The Add HCP modal is removed
// (there is no create-doctor endpoint reachable from this MIS aggregate).
...
const ALL_MR = "All Assigned MRs";
const ALL_SPECIALTIES = "All Specialties";
const ALL_GIFT = "All Gift Status";
const ALL_STATUS = "All Coverage Status";

const GIFT_OPTIONS = [ALL_GIFT, "Over Threshold", "Within Threshold"];

const STATUSES: { label: string; value: "all" | CoverageStatus }[] = [
  { label: ALL_STATUS, value: "all" },
  { label: "Target Met (4+ visits)", value: "Target Met" },
  { label: "On Track (2-3 visits)", value: "On Track" },
  { label: "Under-visited (1 visit)", value: "Under-visited" },
  { label: "Zero-Visit Gap (0 visits)", value: "Zero-Visit Gap" }
];

const VIEW_TABS = [
  { key: "master", label: "Doctor Coverage Master List" },
  { key: "territory", label: "Assigned MR Breakdown" },
...
// Coverage status derived from real totalVisits — the backend does not
// return a per-doctor visit target/plan, so these buckets are a display
// heuristic (0 / 1 / 2-3 / 4+) rather than a real "planned vs actual" ratio.
function statusFor(totalVisits: number): CoverageStatus {
  if (totalVisits <= 0) return "Zero-Visit Gap";
  if (totalVisits === 1) return "Under-visited";
  if (totalVisits <= 3) return "On Track";
  return "Target Met";
}

function mapRow(row: DoctorCoverageRow): DoctorRow {
  return {
    id: row.doctorId,
    name: row.doctorName,
    initials: initialsOf(row.doctorName),
    specialty: row.specialty || "Not specified",
    assignedMR: row.assignedMR || "Unassigned",
    totalVisits: row.totalVisits,
    lastVisitDate: row.lastVisitDate,
    totalSamples: row.totalSamples,
    totalGifts: row.totalGifts,
    totalGiftValueRs: row.totalGiftValueRs,
    overGiftThreshold: row.overGiftThreshold,
    status: statusFor(row.totalVisits)
  };
}
...
export function AdminDoctorCoverageDashboard({ node, path }: { node: ZiviraTreeNode; path: string[] }) {
  ...
  const [mrFilter, setMrFilter] = useState(ALL_MR);
  const [specialty, setSpecialty] = useState(ALL_SPECIALTIES);
  const [giftFilter, setGiftFilter] = useState(ALL_GIFT);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError("");
      try {
        const response = await apiClient.doctorCoverage(month);
        if (!cancelled) setRows(response.data ?? []);
      } catch (e) {
        if (!cancelled) setError(e instanceof Error ? e.message : "Failed to load doctor coverage data");
      } finally {
  ...
  const doctors = useMemo(() => rows.map(mapRow), [rows]);

  const mrOptions = useMemo(() => [ALL_MR, ...Array.from(new Set(doctors.map((d) => d.assignedMR).filter(Boolean)))], [doctors]);
  const specialtyOptions = useMemo(() => [ALL_SPECIALTIES, ...Array.from(new Set(doctors.map((d) => d.specialty).filter(Boolean)))], [doctors]);
  ...
  const filtered = useMemo(() => {
    ...
    return doctors.filter((d) => {
      if (mrFilter !== ALL_MR && d.assignedMR !== mrFilter) return false;
      if (specialty !== ALL_SPECIALTIES && d.specialty !== specialty) return false;
      if (giftFilter === "Over Threshold" && !d.overGiftThreshold) return false;
      if (giftFilter === "Within Threshold" && d.overGiftThreshold) return false;
      if (effectiveStatusFilter !== "all" && d.status !== effectiveStatusFilter) return false;
      if (!q) return true;
      return (
        d.name.toLowerCase().includes(q) ||
        d.specialty.toLowerCase().includes(q) ||
        d.assignedMR.toLowerCase().includes(q)
      );
    });
  }, [doctors, search, mrFilter, specialty, giftFilter, effectiveStatusFilter]);
  ...
  function handleExport() {
    ...
    downloadCsv(
      ...
      filtered.map((d) => ({
        ...
        "Assigned MR": d.assignedMR,
        "Total Visits": d.totalVisits,
        "Last Visit": d.lastVisitDate ?? "Never",
        "Samples Given": d.totalSamples,
        "Gifts Given": d.totalGifts,
        "Gift Value (Rs)": d.totalGiftValueRs,
        "Over Gift Threshold": d.overGiftThreshold ? "Yes" : "No",
        "Status": d.status
      }))
    );
  }
...
<p className="text-xs text-text-secondary mt-1 max-w-3xl">
            Monitor physician visit counts, sample/gift distribution and MCI gift-value threshold alerts from real DCR-derived coverage data.
          </p>
</div>
<div className="flex items-center gap-2.5">
<select
  className="bg-surface-subtle border border-border-subtle rounded-lg px-2.5 py-2 text-xs font-semibold text-text-secondary focus:outline-none focus:ring-1 focus:ring-brand-500"
  value={month}
...
<div>
<span className="text-[11px] font-bold uppercase tracking-wider text-text-muted">Doctors Loaded</span>
<div className="mt-1 flex items-baseline gap-2">
<span className="text-2xl font-extrabold text-text-primary">{loading ? "…" : doctors.length}</span>
</div>
<p className="text-[11px] text-text-secondary mt-0.5">From doctor-coverage MIS for {MONTH_OPTIONS.find((m) => m.value === month)?.label}</p>
</div>
...
<div className="mt-1 flex items-baseline gap-2">
<span className="text-2xl font-extrabold text-brand-corporate">{doctors.length === 0 ? "—" : `${Math.round((doctors.filter((d) => d.totalVisits > 0).length / doctors.length) * 100)}%`}</span>
<span className="text-xs font-semibold text-text-secondary">{doctors.filter((d) => d.totalVisits > 0).length} / {doctors.length}</span>
</div>
...
<div className="flex items-start justify-between">
<div>
<span className="text-[11px] font-bold uppercase tracking-wider text-text-muted">Average Visits / Doctor</span>
<div className="mt-1 flex items-baseline gap-2">
<span className="text-2xl font-extrabold text-text-primary">{doctors.length === 0 ? "—" : (doctors.reduce((sum, d) => sum + d.totalVisits, 0) / doctors.length).toFixed(1)}</span>
<span className="text-xs text-text-secondary">visits / doctor / mo</span>
</div>
<p className="text-[11px] text-text-secondary mt-0.5">Computed from real DCR-derived visit counts</p>
</div>
...
<div className="mt-1 flex items-baseline gap-2">
<span className="text-2xl font-extrabold text-rose-700">{zeroVisitCount}</span>
<span className="text-[11px] font-bold text-rose-700 bg-rose-50 px-1.5 py-0.5 rounded border border-rose-200">{doctors.length === 0 ? "0%" : `${Math.round((zeroVisitCount / doctors.length) * 100)}% GAP`}</span>
</div>
...
<button type="button" className="font-bold text-brand-corporate hover:underline" onClick={() => setDetail({ title: "Dispatch Gap Reminder", body: `${zeroVisitCount} doctors currently have a zero-visit coverage gap this cycle. Session-only — there is no reminder-dispatch write endpoint yet.` })}>Dispatch Gap Reminder →</button>
</div>
</div>
</section>
...
<input className="w-full pl-8 pr-3 py-1.5 bg-surface-subtle border border-border-subtle rounded-lg text-xs placeholder:text-text-muted focus:outline-none focus:ring-1 focus:ring-brand-500" placeholder="Filter by Doctor Name, Specialty, MR..." type="text" value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }}/>
</div>
{/* Assigned MR Dropdown */}
<select className="bg-surface-subtle border border-border-subtle rounded-lg px-2.5 py-1.5 text-xs text-text-secondary font-medium focus:ring-1 focus:ring-brand-500" value={mrFilter} onChange={(e) => { setMrFilter(e.target.value); setPage(1); }}>
{mrOptions.map((t) => <option key={t} value={t}>{t}</option>)}
</select>
{/* Specialty Dropdown */}
...
{/* Gift Threshold Dropdown */}
<select className="bg-surface-subtle border border-border-subtle rounded-lg px-2.5 py-1.5 text-xs text-text-secondary font-medium focus:ring-1 focus:ring-brand-500" value={giftFilter} onChange={(e) => { setGiftFilter(e.target.value); setPage(1); }}>
...
<th className="py-3 px-4">Gift Value &amp; Threshold</th>
<th className="py-3 px-4">Assigned Field Rep</th>
<th className="py-3 px-4">Total Visits</th>
<th className="py-3 px-4">Last Visit</th>
<th className="py-3 px-4">Samples &amp; Gifts</th>
...
{!loading && pageRows.length === 0 && (
  <tr>
    <td colSpan={7} className="py-10 px-4 text-center text-text-muted text-xs">No doctors match the current search/filters.</td>
  </tr>
)}
{loading && (
  <tr>
    <td colSpan={7} className="py-10 px-4 text-center text-text-muted text-xs">Loading doctor coverage data…</td>
  </tr>
)}
{pageRows.map((d) => {
      ...
      <td className="py-3.5 px-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-[11px] text-text-secondary">{d.specialty}</div>
          </div>
        </div>
      </td>
      <td className="py-3.5 px-4">
        <div className="font-semibold text-text-primary">₹{d.totalGiftValueRs.toLocaleString("en-IN")}</div>
        <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-extrabold border ${d.overGiftThreshold ? "bg-rose-100 text-rose-700 border-rose-200" : "bg-emerald-100 text-emerald-800 border-status-success-bg"}`}>
          {d.overGiftThreshold ? "Over Threshold" : "Within Threshold"}
        </span>
      </td>
      <td className="py-3.5 px-4">
        <div className="font-semibold text-text-primary">{d.assignedMR}</div>
      </td>
      <td className="py-3.5 px-4">
        <span className={isZero ? "font-bold text-rose-600" : "font-semibold text-text-primary"}>{d.totalVisits}</span>
      </td>
      <td className="py-3.5 px-4">
        <div className={isZero ? "font-bold text-rose-700" : "font-medium text-text-primary"}>{d.lastVisitDate ?? "Never visited"}</div>
      </td>
      <td className="py-3.5 px-4">
        <div className="text-[11px] text-text-secondary">{d.totalSamples} samples • {d.totalGifts} gifts</div>
      </td>
        ...
        <div className="flex items-center justify-end gap-2">
          ...
          {isZero ? (
            <button
              ...
              title="Trigger Alert"
              onClick={() => setDetail({ title: `Escalate — ${d.name}`, body: `${d.name} has had zero visits in the current cycle. Escalating notifies ${d.assignedMR}. Session-only — there is no escalation write endpoint yet.` })}
            >
              Escalate
            </button>
          ) : (
            <button
              ...
              onClick={() => setDetail({ title: `${d.name} — Call Details`, body: `${d.specialty}. ${d.totalVisits} visits logged this cycle. Assigned MR: ${d.assignedMR}. Last visit: ${d.lastVisitDate ?? "Never"}. Samples: ${d.totalSamples}, Gifts: ${d.totalGifts} (₹${d.totalGiftValueRs.toLocaleString("en-IN")}${d.overGiftThreshold ? " — over MCI gift threshold" : ""}).` })}
            >
