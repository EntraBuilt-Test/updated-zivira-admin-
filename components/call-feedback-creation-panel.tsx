...
const MASTER_KEY = "callFeedbackCreation";

type Row = { id: string; feedbackText: string };
...
// Matches sanpharma.info's Options >> Call Feedback Creation screen
// (App_CallFeedback.aspx) exactly: before any feedback exists, just an
// "Add Feedback" button next to a text box; typing a value and clicking
// Add Feedback saves it and flips the page into a bordered
...
export function CallFeedbackCreationPanel({ masterKey: _masterKey }: { masterKey: string }) {
  ...
  async function loadRows() {
    ...
    try {
      ...
      setRows(res.data.map((r: MasterRecord) => ({ id: r.id, feedbackText: String(r.feedbackText ?? "") })));
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to load Call Feedback Creation");
    } finally {
      setLoading(false);
    }
  }
  ...
  function editText(id: string, value: string) {
    setRows((prev) => prev.map((r) => (r.id === id ? { ...r, feedbackText: value } : r)));
  }
  ...
  async function addFeedback() {
    ...
    setError(null);
    try {
      await apiClient.createMasterRecord(MASTER_KEY, { feedbackText: newText.trim(), status: "Active" });
      setNewText("");
      await loadRows();
    } catch (err) {
  ...
  async function saveAll() {
    ...
    try {
      for (const row of rows) {
        await apiClient.updateMasterRecord(MASTER_KEY, row.id, { feedbackText: row.feedbackText });
      }
      setNotice("Call Feedback saved successfully.");
      await loadRows();
    } catch (err) {
  ...
  if (loading) return <div className="card p-4 text-sm">Loading Call Feedback Creation...</div>;

  return (
    <div className="space-y-4">
      <div className="card p-4 text-center">
        <h2 className="text-lg font-semibold mb-4">Call Feedback Creation</h2>
