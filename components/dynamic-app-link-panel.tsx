...
// Matches sanpharma.info's Options >> Menu Creation screen
// (Menu_Creation.aspx) exactly, reused here to back "Dynamic App Link":
// before anything exists, just an "Add Dynamic App Link" button; clicking
// it opens the Menu Type / Menu Name / Path / Logo form with Save/Clear,
// and once at least one link has been saved a bordered
// S.No/Menu type/Menu name/Edit/Deactivate table appears below it.
    ...
    <div className="space-y-4">
      <div className="card p-4 text-center">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold">Dynamic App Link</h2>
          <button
            onClick={openAdd}
            style={{ border: "1px solid #94a3b8", borderRadius: 6, background: "#e0f2fe", padding: "8px 16px", fontWeight: 600 }}
          >
            Add Dynamic App Link
          </button>
        </div>

        {error && <div className="text-sm text-red-600 mb-3 text-left">{error}</div>}
        {notice && <div className="text-sm text-green-700 mb-3 text-left">{notice}</div>}

        {formOpen && (
          <div className="flex flex-wrap items-end justify-center gap-4 mb-6 text-left">
            <div>
